			...
			<div class="media-body">
				...
				<ul class="list-inline">
					<li v-if="$root.user.authenticated">
						<a href="#" @click="toggleReplyForm(comment.id)">{{ replyFormVisible === comment.id ? 'Cancel' : 'Reply'}}</a>
					</li>
					<li>
				...
				<div class="video-comment clearfix" v-if="replyFormVisible === comment.id">
					...
					<div class="pull-right">
						<button type="submit" class="btn btn-default" @click.prevent="createReply(comment.id)">Submit</button>
					</div>
				</div>
